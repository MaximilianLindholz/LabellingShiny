raster3d_animation_UI <- function(id) {
  ...
  tagList(
    ...
    fluidRow(
      column(4, align = "center", uiOutput(ns("z_slider_UI")))
    ),
    fluidRow(
      column(12, align = "center", plotOutput(ns("z_plot")))
    )
  )
}
...
raster3d_animation_Module <- function(input, output, session, im) {
  ...
  output$z_plot <- renderImage({
    ...
    list(
      ...
      width = rv$zoom * 400,   # Change this to control the base size
      height = rv$zoom * 400  # Change this to control the base size
    )
  }, deleteFile = F)
